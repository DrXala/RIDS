...
check_gpio_point_monitoring(){
	...
	do
		...
		debug  "The GPIO ${monitoring_gpio_number} has a value of : ${monitoring_gpio_current_state} \
			// when the access is closed, it must return a value of : ${monitoring_gpio_value_access_closed}, \
			and it has a tempo trigger time of : ${tempo_trigger_alarm}" 
		
		if [[ "${monitoring_gpio_current_state}" -eq "${monitoring_gpio_value_access_closed}" && "${gpio_open_flag}" -eq 1 ]]
		then
			event_log "door_closed.png" "The access ${monitoring_gpio_name} is detected as closed"
			sql_request_RW "UPDATE GPIO SET monitoring_gpio_current_state = '${monitoring_gpio_current_state}', \
				gpio_open_flag = '0'"
		fi
		
		# If the currently checked access point is opened, and is a temporized access,
		# and the alarm is not idle or being armed, then someone is entering a monitored area and has 
		# to authenticate quickly. Menwhile the alarm is waiting for authentication
		if [[ "${monitoring_gpio_current_state}" -ne "${monitoring_gpio_value_access_closed}" \
		&& "${tempo_trigger_alarm}" -ne 0 \
		&& ! "${alarm_status}" =~ ('0'|'1') \
		&& "${gpio_open_flag}" -ne 1 ]]
		then
			alarm_status=3
			debug "The access point linked to the GPIO : ${monitoring_gpio_number}, \
				named : ${monitoring_gpio_name}, is detected as open. We proceed to the alarm status 3"
			event_log "alarm_state_3.png" "The access ${monitoring_gpio_name} triggered the temporisation of \
				the alarm!!!"
			# capture_image_cctv
		fi
		
		# If the currently checked access point is opened, and is NOT a temporized access,
		# and the alarm is not idle or being armed, then an intrusion is confirmed 
		if [[ "${monitoring_gpio_current_state}" -ne "${monitoring_gpio_value_access_closed}" \
		&& "${tempo_trigger_alarm}" -eq 0 \
		&& ! "${alarm_status}" =~ ('0'|'1') \
		&& "${gpio_open_flag}" -ne 1 ]]
		then
			alarm_status=4
			...
			debug "The access point linked to the GPIO : ${monitoring_gpio_number}, \
				named : ${monitoring_gpio_name}, is detected as open. We proceed to the alarm status 4" 
		fi
		
		if [[ "${monitoring_gpio_current_state}" -ne "${monitoring_gpio_value_access_closed}" 
		&& "${gpio_open_flag}" -eq 0 ]]
		then
			event_log "door_open.png" "The access ${monitoring_gpio_name} is detected as open"
			sql_request_RW "UPDATE GPIO SET monitoring_gpio_current_state = '${monitoring_gpio_current_state}', \
				gpio_open_flag = '1'"
		...
		&& "${1}" == "pre_arming_check" ]]
		then
			if [[ "${mandatory_closed_access_on_arming}" -eq 1 ]]
			then
				debug "Mandatory closed access point for arming is found opened! GPIO number \
					${monitoring_gpio_number} returns the access is still open, it should have \
					a value of: ${monitoring_gpio_value_access_closed}, but instead we find a \
					value of ${monitoring_gpio_current_state} " 
				...
				event_log "door_still_opened.png" "Warning! The alarm is arming but the access \
					${monitoring_gpio_name} is still detected as opened"
			else
				debug "GPIO number ${monitoring_gpio_number} returns the access is still open, \
					it should have a value of: ${monitoring_gpio_value_access_closed}, \
					but instead we find a value of ${monitoring_gpio_current_state} " 
				event_log "door_still_opened.png" "Warning! The alarm is arming but the access \
					${monitoring_gpio_name} is still detected as opened"
				sound_player "${audio_signal_type}" alterna_3 notification
...
# Function made to send SMS based on a list of recipients in the database, 
# and the SMS API from OVH (paid functionnality, transiting through the web).
send_sms(){
	# We retrieve the datas for every recipient for who the setting 'SEND_SMS' is set to 1, 
	# and we send a SMS which content is based on the event which triggered the function.
	if [[ "${sms_sent}" -ne 1 ]]
	then
		while read -ra row
		do
			...
			LAST_NAME="${row[2]}"
			SEND_SMS="${row[3]}"
			#SEND_EMAIL="${row[4]}" # var unused...
			SMS_NUMBER="${row[5]}"
			#EMAIL_ADDRESS="${row[6]}" # var unused...
			recipient_name="${FIRST_NAME}_${LAST_NAME}"
			# Based on the argument {1} retrieved from the function, we fetch the body of the sms 
			# and we send it to the recipient
			sms_message_body=$(grep "${1}" "language/${language}/sms_messages.txt" | awk -F '=' '{ print $2 }' )
			if [[ "${sms_sent}" -ne 1 && "${SEND_SMS}" -eq 1 ]]
			then
				php bin/OVH_API/php-ovh-dep/sms-sender.php "${SMS_NUMBER}" "${sms_message_body}" & disown >> /dev/null 2>&1						
			fi
...
rfid_reader(){ 
# We make sure to reset the potential previous results of RFID readings
	rfid_reader_result=0
	valid_rfid_detected='u'
	if [[ $SECONDS -lt $RFID_reader_resting ]]
	then
		echo "" > rfid_reader_capture.txt
		debug "We are still in the resting time of the RFID, \
			we reset the file content so no further RFIS scan is possible until the rest time is reached."
	fi
	...
	then
		valid_rfid_request=$(sql_request_RO "select * from RFID WHERE rfid_card_ID = ${rfid_reader_capture} \
			AND rfid_card_flag <>'disabled'")
		if 	[[ -z "${valid_rfid_request}" ]] 
		then
			if	[[ "${alarm_status}" -eq 5 ]]
			then
				debug "A new RFID card has been in contact with the RFID reader" 
				sql_request_RW "INSERT INTO RFID (ID, rfid_card_ID, attribution_first_name, \
					attribution_last_name, rfid_card_flag ) VALUES ( NULL, '${rfid_reader_capture}', \
					'TO_CUSTOMIZE', 'TO_CUSTOMIZE', 'active')"
				sound_player "${audio_signal_type}" message_alarm_rfid_added
				event_log "added_rfid_card.png" "New RFID added with success."
			else
				debug "Unknown RFID!: ${rfid_reader_capture}" 
			...
			rfid_reader_result=1
			sound_player "${audio_signal_type}" alterna_1 notification
			debug "function rfid not void : last_alarm_known_status = ${last_alarm_known_status} AND \
				We found a known RFID : @${valid_rfid_request}@ "
			valid_rfid_detected=1
			# we reset the number of detected password attemps (password or RFID) 
			# since a valid one has been detected
			password_attempt=0
			...
			then
				debug "That RFID card is in the database but has no name attributed. \
					We can't let an access to a card which has no attribution name for security reasons."
				sound_player "${audio_signal_type}" alterna_2 notification
				sound_player "${audio_signal_type}" message_alarm_rfid_has_no_name	
				valid_rfid_detected=0
				event_log "wrong_rfid_card.png" "The RFID card number ${rfid_card_ID} was detected and \
					is part of the database, but has no user attributed to it.\
					We can't let an access to a card which has no attribution name for security reasons"
			fi
			if [[ "${rfid_card_flag}" =~ (lost|stolen) ]]
			then
				debug "That RFID card ${rfid_card_ID} is detected, is in the database but \
					the ID is referenced as stolen or lost!."
				valid_rfid_detected=0
				if [[ "${trigger_alarm_on_lost_or_stolen_card}" -eq 1 ]]
				then
					sound_player "${audio_signal_type}" alterna_2 intrusion
					sound_player "${audio_signal_type}" message_alarm_rfid_revoked	
					alarm_status=4
					event_log "wrong_rfid_card.png" "The RFID card number ${rfid_card_ID} was detected and \
						is referenced as lost or stolen! Alarm is triggered!"
				else
					sound_player "${audio_signal_type}" alterna_2 notification
					sound_player "${audio_signal_type}" message_alarm_rfid_revoked	
					event_log "wrong_rfid_card.png" "The RFID card number ${rfid_card_ID} was detected and \
						is referenced as lost or stolen!"
				fi							
			fi
			if [[ "${alarm_status}" -eq 5 ]]
			then
				debug "We noticed that the RFID card @${valid_rfid_request}@ attributed to ${rfid_card_ID} \
					already exists" 
				sound_player "${audio_signal_type}" message_alarm_rfid_already_exists	
			fi
			if [[ "${alarm_status}" -eq 4 && "${valid_rfid_detected}" == 1 ]]
			then
				debug "We kill the piezo alarm" 
				pkill -f piezo
			...
			then
				sound_player "${audio_signal_type}" message_alarm_password_success
				alarm_status=0
				event_log "alarm_unlocked.png" "The RFID attributed to ${rfid_attribution} successfully \
					unlocked the alarm"
			fi
			...
			if [[ "${alarm_status}" -eq 0 && "${valid_rfid_detected}" == 1 \
			&& "${arming_cancellation}" -ne 1 && "${triggered_alarm}" -ne 1 \
			&& "${last_alarm_known_status}" -ne 2 && "${last_alarm_known_status}" -ne 3 \
			&& "${last_alarm_known_status}" -ne 4 ]]
			then
				alarm_status=1
				password_attempt=0
				echo "" > rfid_reader_capture.txt
				event_log "alarm_armed.png" "The RFID attributed to ${rfid_attribution} successfully \
					armed the alarm"
			fi
			...
			then
				triggered_alarm=0
				alarm_status=0
				event_log "alarm_siren_off.png" "The RFID attributed to ${rfid_attribution} successfully \
					shut down the sonore alarm"
			fi
			echo "" > rfid_reader_capture.txt
			sudo systemctl restart rfid_reader.service
			RFID_reader_resting=$((SECONDS+10))
			debug "We apply a no RFID scan timezone.  The system time is $SECONDS, \
				and the next system tme where the scan will be available is : ${RFID_reader_resting}"
		fi
	else
		debug "No RFID card seems to have been in contact with the reader" 
	fi
}

# Function made to play pre-recorded TTS voices, made to announce what the alarm is doing,
# and have a better user experience.
#  - This function is available only if in the settings, the value "silent_voice" is set to 0, 
#    which means we don't make it silent.
#  - Based on the setting value 'language' the voices are played in the selected language. 
#    If no language suits you, you are able to create it.
...
sound_player(){
	...
	then
		if [[ ! "${alarm_status}" =~ ^('3'|'4')$ ]]
		then
			omxplayer --no-keys -o local "language/${language}/${2}.wav" \
				Audio codec pcm_s16le channels 1 samplerate 16000 bitspersample 16 > /dev/null 2>&1
			debug "We play ${2}.wav"
		else
			omxplayer --no-keys -o local "language/${language}/${2}.wav" \
				Audio codec pcm_s16le channels 1 samplerate 16000 bitspersample 16 > /dev/null 2>&1 &
	...
	then
		if [[ "${3}" == "notification" && "${silent_buzzer}" -eq 0 && "${gpio_piezo_number}" -ne 99 ]]
		then
			./piezo_alarm_sound.sh "${2}" "${gpio_piezo_number}" & disown
		fi
...
# Function made to initiate necessary items and GPIOs related to the way of the users have to 
# authenticate (RFID, external system link with a dry_contact, etc.)
initiate_user_input(){
	if [[ "${user_input_method}" == "dry_contact" ]]
	then
		gpio_user_input_method=$(sql_request_RO "select gpio_user_input_method from SETTINGS")
		gpio_user_input_method_circuit_mode=$(sql_request_RO "select gpio_user_input_method_circuit_mode \
			from SETTINGS")
		#declare -n ref="opened_access_gpio_${gpio_user_input_method}" # var unused...
		#ref=0 # var unused...
		gpio -g mode "${gpio_user_input_method}" OUT
		if [[ "${gpio_user_input_method_circuit_mode}" == "1" ]]
		then
			gpio -g write "${gpio_user_input_method}" 1
		else
			gpio -g write "${gpio_user_input_method}" 0
		fi
	fi
}
...
capture_image_cctv(){
	...
	ffmpeg -loglevel fatal -rtsp_transport tcp \
		-i "${video_capture_url}" \
		-r 1 -vframes 1 "${image_capture_folder}${outfilename}"
    #function sendmail avec piece jointe image
}

# Function set to capture the stream from a Ip Camera, streaming with RTSP functionnality
# This function is available only if in the settings, the value "video_capture_enabled" is set to 1
# By definition, if their is an IP Camera available and linked to the system, everytime an 'alert' event is 
# detected, a video capture is made. Only if their is a notification kind of event and 
# the 'video_capture_on_alert_only' value is set to 1 in the settings, then no video is made.
capture_video_cctv(){
	if [[ "${video_capture_enabled}" -eq 1 ]]
	then
		# Setting up the name of the video clip
		cctv_actual_filename=$(date +%Y-%m-%d_%H-%M-%S.mp4)

		# If the argument is NOT 'alert' AND if the setting 'video_capture_on_alert_only' is set to 0 (disabled), 
		# then we capture the stream and put it in a video file, and we upload the datas to the database 
		# to be able to retrieve it in the WebUI
		if [[ "${1}" != "alert" && "${video_capture_on_alert_only}" -eq 0 ]]
		then
			nohup ffmpeg -t "${video_capture_timing}" \
				-i rtsp://"${video_capture_username}":"${video_capture_password}"@"${video_capture_url}" \
				-c copy -map 0 -f segment -segment_time 600 -segment_format mp4 \
				-strftime 1 "cctv_captures/${cctv_actual_filename}" -s '1920x1080' &
			sql_request_RW "INSERT INTO CCTV_CAPTURES ( ID, FILENAME, TRIGGERING_EVENT ) VALUES \
				( NULL, '${cctv_actual_filename}', '${1}' )"
		fi    

		# If the argument is 'alert', then we capture the stream and put it in a video file, 
		# and we upload the datas to the database to be able to retrieve it in the WebUI
		if [[ "${1}" = "alert" ]]
		then
			nohup ffmpeg -t "${video_capture_timing}" \
				-i rtsp://"${video_capture_username}":"${video_capture_password}"@"${video_capture_url}" \
...
kill_specific_job(){
	jobs | grep "${1}" | awk '{print substr($1,2,1)}' | while IFS= read -r job_number_to_kill
	do
		kill "%${job_number_to_kill}"
	done
}

# Function to write datas in a .txt debug file 
# This function is available only if in the settings, the value "debug_activated" is set to 1
debug(){
	if [[ "${debug_activated}" -eq 1 ]]
	then
		date +%Y_%m_%d__@__%H:%M:%S" / ${1}" >> "${debug_path}" 2>&1
	fi
}

event_log(){
	sql_request_RW "INSERT INTO EVENT_LOG ( ID, LOG_TIMESTAMP, EVENT_LOG_ILLUSTRATION, LOG_CONTENT ) VALUES \
		( NULL, NOW(), '${1}', '${2}' )"
}
...
check_timestamp_last_sms(){
	date_present=$(date +%s)
	date_sms=$(timestamp_last_sms)
	result=$(( "$date_present" - "$date_sms"))
	echo $result
}
...
global_settings_load_up(){
	...
	# If the GPIO is not 99, then their is a piezo connected and we have to initiate it.
	if [[ "${gpio_piezo_number}" -ne 99 ]]
	then
...
# This is the point where we check if the system was rebooted less than 2 minutes ago and if a SMS
# was already sent or not.
# Since the Rpi is not supposed to be rebooted, and if the setting value 'send_sms_on_reboot' is set
# to 1 (enable), then we send a SMS because their was an unexpected event which might be sabotage
...
# Everytime the Rasberry restart or the service restart, we need to know it.
# So we add this as an event to the event log.
event_log "alarm_restart_loading.png" "The alarm finished to load the parameters, \
	the alarm is starting now in the status ${default_alarm_status}"
...
do
	...
	debug "rfid_reader_result = ${rfid_reader_result} \
		// alarm_status = ${alarm_status} \
		// last_alarm_known_status = ${last_alarm_known_status} \
		// password_attempt = ${password_attempt} \
		// sms_sent = ${sms_sent}"
	arming_cancellation=0

	# This is the alarm state where the alarm is not active and waiting 
	# for being armed or being under maintenance.
	case "${alarm_status}" in
		0)
			sleep 0.8
			debug "We are in the status 0" 
			debug "At the moment, the alarm status is : ${alarm_status}, and at the moment, \
				the rfid reader return value is : ${rfid_reader_result}" 
			...
			case "${override_mode}" in
				...
				2)	sound_player "${audio_signal_type}" alterna_1 notification
					...
					alarm_status=2
					sql_request_RW "UPDATE SETTINGS SET central_mode_override = '0'"
					event_log "alarm_monitoring.png" "Alarm was armed manually from the WebUI. \
						The alarm is now activated and monitoring."
					;;
			esac
			...
			# This is the alarm state where the alarm is arming itself.
			# The arming consists in making a self diagnostic of all the access point state, 
			# to make sure they are all closed, and let the user get out of the house before considering
			# any opening of an access point being an possible intrusion
		1)
			sleep 0.7
			...
			then
				...
				debug "It is ${SECONDS} in the system. With the variable ${alarm_set_on_delay}, \
					we predict arming temporisation end time in ${alarm_arming_end}" 
				...
				do
					...
					if [[ "${rfid_reader_result}" -eq "1" ]]
					then
						arming_cancellation=1
						sound_player "${audio_signal_type}" message_alarm_arming_canceled
						rfid_reader_result=0
						debug "Alarm arming canceled command received" 
						event_log "alarm_unlocked.png" "Alarm arming canceled command received from RFID \
							attributed to ${rfid_attribution}"
						alarm_status=0
				...
				event_log "alarm_unlocked.png" "Alarm arming canceled command received because \
					an access point configured as mandatory closed for arming is still detected as opened"
				alarm_status=0
			fi
			;;
		...
		2)
			...
			then
				...
				then
					...
					event_log "wrong_rfid_card.png" "Unsuccessful attempt to unlock the alarm with \
						RFID attributed to ${rfid_attribution}"
					# capture_image_cctv
				fi
		...
		# This is the alarm state where the alarm detected the opening of a temporised access point
		# (ie :the front door), and is waiting for the user to authenticate as a legit human, 
		# getting inside the protected perimeter.
		3)
			sleep 0.6
			...
			debug "It is ${SECONDS} in the system. With the variable ${alarm_temporisation_delay}, \
				we predict a temporisation end time at ${open_tempo_end}" 
			...
			do
				...
				then
					sound_player "${audio_signal_type}" alterna_4 notification
					debug "An acces point was detected as open, and still, no one has disabled the alarm, \
						we are sending a last call sound if the legit user forgot to disable the alarm"
				fi
				...
				then
					...
					if [[ "${rfid_reader_result}" -eq 1 ]] 
					then
						debug "A valid RFID car have been checked, we disarm the alarm."
						led_status green 2 & disown
						alarm_status=0
						break
					fi
		...
		# This is the alarm state where the alarm is under maintenance,
		# letting you see and modify the settings.
		5)
			sleep 1
			...
			then
				debug "We leave the management mode"
				sound_player "${audio_signal_type}" alterna_1 notification
				alarm_status=0
